...
export const useMemoriesStore = defineStore('memoriesStore', () => {
  const memories = ref([
    {
      id: 'm1',
  ...
  function memory(memoryId) {
    return memories.value.find((memory) => memory.id === memoryId)
  }

  function addMemory(memoryData) {
    let newMemory = { ...memoryData, id: new Date().toISOString() }
